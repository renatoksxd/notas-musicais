...
Atributes:
    ESCALAS: Escalas emplementadas usando a notação de inteiros
    NOTAS: Notas musicais

# Escalas

As escalas são implementadas em uma constante chamada `ESCALAS`. Que é um dicionário onde as chaves são as escalas. Se quiser ver todas as escalas implementadas pode usar:

```py tittle="No seu shell interativo"
...
A notação inteira para as escalas foi retirada da página [List of musical scales and modes](https://en.wikipedia.org/List_of_musical_scales_and_modes) na Wikipedia.

top: Dica!
    Você pode contribuir com novas escalas usando a notação inteira:
    [Escalas wikipedia](https://en.wikipadia.org/wiki/List_of_musical_scales_and_modes).
